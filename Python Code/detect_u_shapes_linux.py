...
def find_u_shapes(img, debug_mode):
    ...
    im2, cnts, h = cv2.findContours(mask.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    for contour in cnts:
        ...
        if peri > kMIN_PERIMETER:
            ...
            im3, cnts_inner, h3 = cv2.findContours(inner_img.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
